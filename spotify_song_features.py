...
st.write("Predict Spotify track popularity using ML + Deep Learning with EDA, tuning, and SHAP.")
...
PAGE = st.sidebar.radio(
    "Go to",
    ["🏠 Home", "📊 EDA", "🧪 Train", "🔮 Predict", "📈 Compare", "🧠 Explain (SHAP)", "⚙️ Settings"],
    index=0,
)
...
def safe_model_pickle(obj: Any) -> bytes:
...
if PAGE == "🧪 Train":
    ...
    tune = st.sidebar.toggle("Enable Hyperparameter Tuning", value=True)
    cv_folds = st.sidebar.slider("CV folds", 3, 10, 5, 1)
    n_iter = st.sidebar.slider("Random Search iterations", 5, 50, 10, 5)
    n_jobs = st.sidebar.selectbox("n_jobs", [-1, 1, 2, 4], index=0)
    ...
    if train_btn:
        ...
        if st.button("💾 Download trained model (pickle)"):
            blob = safe_model_pickle(st.session_state["bundle"])
            st.download_button(
...
if PAGE == "🧠 Explain (SHAP)":
    ...
    if not SHAP_AVAILABLE:
        st.error("SHAP is not installed. Install it: `pip install shap` and restart Streamlit.")
        st.stop()

    if not is_tree_model(model_choice):
        st.info("SHAP here is configured for tree-based models. Train a tree model (RF/ExtraTrees/GB/XGB/LGBM/CatBoost).")
        st.stop()

    # Use a sample to keep it fast
    X_sample = X_train.sample(min(200, len(X_train)), random_state=int(seed))

    st.write("Generating SHAP values (may take a moment)...")
...
# ============================================================
st.markdown("---")
st.caption("Built by Marco Ortiz | Spotify Popularity Prediction using ML & Deep Learning")