...
if PAGE == "📈 Compare":
    ...
    hist_df["composite_score"] = (
        hist_df["r2"]
        - 0.01 * hist_df["rmse"]
        - 0.005 * hist_df["train_seconds"]
    )
    
    best_overall = hist_df.sort_values("composite_score", ascending=False)
    
    st.markdown("### 🥇 Best Overall Model (Composite Score)")
    st.caption(
        "Composite score balances accuracy (R², RMSE) and efficiency (training time)."
    )
    
    st.dataframe(best_overall.head(5), use_container_width=True)
    ...
    c1.write("**Lowest RMSE**")
    c1.dataframe(hist_df.sort_values("rmse").head(3))

    c2.write("**Lowest MAE**")
    c2.dataframe(hist_df.sort_values("mae").head(3))

    c3.write("**Highest R²**")
    c3.dataframe(hist_df.sort_values("r2", ascending=False).head(3))

    c4.write("**Lowest MAPE (%)**")
    c4.dataframe(hist_df.sort_values("mape").head(3))

    st.markdown("### ⏱️ Training Speed Comparison")
    fig = px.bar(
        hist_df,
        x="model_name",
        y="train_seconds",
